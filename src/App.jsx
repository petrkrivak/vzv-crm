...
const getStoredSession = () => {
  try { const s = localStorage.getItem("vzv_session"); return s ? JSON.parse(s) : null; }
  catch { return null; }
};
...
const NOTE_TYPES = {
  email: { label: "Email", bg: "#F0F8EC", color: "#236D00", stroke: "#2E8B00", border: "#C8E6B0", icon: ["M4 4h16a2 2 0 012 2v12a2 2 0 01-2 2H4a2 2 0 01-2-2V6a2 2 0 012-2z", "M22 6l-10 7L2 6"] },
  tel: { label: "Tel", bg: "#FFF8E1", color: "#7A5200", stroke: "#B8860B", border: "#F5D580", icon: "M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.8a19.79 19.79 0 01-3.07-8.67A2 2 0 012 .18h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" },
  sch: { label: "Sch", bg: "#F0F8EC", color: "#236D00", stroke: "#2E8B00", border: "#C8E6B0", icon: ["M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2", "M9 7a4 4 0 100 8 4 4 0 000-8z", "M23 21v-2a4 4 0 00-3-3.87", "M16 3.13a4 4 0 010 7.75"] },
  nab: { label: "Nab", bg: "#FFF8E1", color: "#7A5200", stroke: "#B8860B", border: "#F5D580", icon: ["M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z", "M14 2v6h6", "M16 13H8", "M16 17H8"] },
};
const LONG_NOTE = 120;
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      <input style={{ ...inp, paddingLeft: 34 }} value={value}
        ...
        onFocus={() => { if (value.length === 0) { setFilteredSugg(suggestions); setOpen(suggestions.length > 0); } }}
        ...
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 200, background: C.white, border: `1px solid ${C.border}`, borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,0.12)", maxHeight: 220, overflow: "auto", marginTop: 2 }}>
          {filteredSugg.map((item, i) => (
            <div key={i} onMouseDown={() => { onChange(item); setOpen(false); }}
              style={{ padding: "10px 14px", cursor: "pointer", fontSize: 15, color: C.text, borderBottom: i < filteredSugg.length - 1 ? `1px solid ${C.border}` : "none" }}
...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      storeSession(data); onLogin(data);
    } catch { setError("Nesprávný email nebo heslo"); }
  ...
  return (
    <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${C.accentLight} 0%, ${C.bg} 50%, ${C.yellowLight} 100%)`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter',system-ui,sans-serif", padding: 20 }}>
      ...
      <div style={{ background: C.white, borderRadius: 20, padding: "40px 36px", width: "100%", maxWidth: 400, boxShadow: "0 8px 40px rgba(0,0,0,0.12)" }}>
        ...
        {error && <div style={{ background: `${C.danger}10`, border: `1px solid ${C.danger}30`, borderRadius: 8, padding: "10px 14px", marginBottom: 16, fontSize: 13, color: C.danger, fontWeight: 500 }}>⚠ {error}</div>}
        <div style={{ marginBottom: 14 }}>
          <label style={s.label}>Email</label>
          <input style={inp} type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="vas@email.cz" onKeyDown={e => e.key === "Enter" && handleLogin()} />
        </div>
        <div style={{ marginBottom: 24 }}>
          <label style={s.label}>Heslo</label>
          <div style={{ position: "relative" }}>
            <input style={{ ...inp, paddingRight: 44 }} type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" onKeyDown={e => e.key === "Enter" && handleLogin()} />
  ...
  const createUser = async () => {
    ...
    try {
      const res = await fetch(`${SUPABASE_URL}/auth/v1/admin/users`, {
        ...
        body: JSON.stringify({ email: form.email, password: form.password, email_confirm: true, user_metadata: { full_name: form.full_name, role: form.role } }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.msg || "Chyba vytvoření uživatele");
      await sb(`profiles`, { method: "POST", body: JSON.stringify({ id: data.id, email: form.email, full_name: form.full_name, role: form.role, region: form.region }), headers: { "Prefer": "resolution=merge-duplicates,return=representation" } }, session.access_token);
      setSuccess(`Uživatel ${form.full_name} byl vytvořen.`);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <button onClick={() => { setModal(true); setError(""); setSuccess(""); }} style={s.btn("primary")}><Icon d={Icons.plus} size={14} />Přidat uživatele</button>
      </div>
      ...
      {modal && (
        <Modal title="Nový uživatel" onClose={() => setModal(false)} onSave={createUser} saveLabel={loading ? "Vytvářím…" : "Vytvořit uživatele"} saveDisabled={loading}>
  ...
  return (
    <div style={{ textAlign: "center", padding: "8px 0" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <a href={link} target="_blank" rel="noreferrer" onClick={onClose} style={{ ...s.btn("gcal-big"), textDecoration: "none", justifyContent: "center", width: "100%", boxSizing: "border-box" }}>
...
const GREETINGS = {
  po: {
    rano: ["Pondělní ráno — někdo to dělat musí, a ten někdo jsi ty.", "Nový týden, čistý stůl, plná hlava plánů. Pojď na to!", "Pondělí zavolalo. Řeklo, že máš plno schůzek.", "Začínáme znova od nuly — ale tentokrát chytřeji.", "Káva v ruce, CRM otevřený. To je správný start týdne.", "Pondělí není problém. Problém jsou zákazníci, kteří ještě nevědí, že tě potřebují.", "Nový týden = nová šance přesvědčit zákazníky, že Hangcha je ta správná volba.", "Vstávej, svět nezastaví — a ani ty bys nechtěl.", "Pondělí: den, kdy se rozhoduje, jak bude vypadat celý týden.", "Týden začíná. Zákazníci čekají, vozíky jsou připraveny."],
    dopoledne: ["Pondělní dopoledne — ideální čas na telefonáty, než zákazníci utečou na oběd.", "Dopoledne v pondělí: všichni jsou ještě v práci, nikdo ještě není naštvaný.", "Zavolej dřív, než tě předběhne konkurence.", "Dopoledne se rozjíždí. Ty taky.", "Pondělní dopoledne patří těm, kdo volají jako první.", "Máš před sebou celý týden. Nejlepší je začít hned.", "Každý telefonát dopoledne je investice do pátečního výsledku.", "Kdo zavolá jako první v pondělí, vyhrává týden.", "Dopoledne v pondělí: konkurence ještě spí, ty už makáš.", "Rozjezd je nejdůležitější část závodu. A ty jsi právě nastartoval."],
    odpoledne: ["Pondělní odpoledne — ještě je čas na jeden důležitý telefonát.", "Dopoledne bylo příprava, odpoledne je akce.", "Zákazníci po obědě bývají přístupnější. Využij to.", "Přišel si do druhé půlky pondělí. Dobré tempo.", "Odpoledne v pondělí: kdo to nevzdal ráno, nevzdá to ani teď.", "Ještě pár hodin do konce dne — udělej z nich něco.", "Pondělní odpoledne patří těm, co dopoledne makali.", "Zbývá půlka dne. Co z ní uděláš?", "Odpoledne je čas uzavřít, co ráno začalo.", "Pondělí se chýlí ke konci. Jak to dopadlo?"],
    vecer: ["Pondělí přežito. A to je vlastně velký úspěch.", "První den týdne za tebou. Zbytek bude snazší.", "Večer v pondělí: čas na přehled, co se povedlo.", "Dobrý start týdne? Dobrý výsledek na konci.", "Pondělí skončilo. Ty jsi skončil líp.", "Zápisky, úkoly, plány — večer v pondělí je čas na pořádek.", "Co jsi dnes udělal, ti zítra usnadní práci.", "Pondělní večer: odměna za přežití prvního dne.", "Sedm dní v týdnu, šest zbývá. Dobrý základ.", "Dobrý obchodník si v pondělí večer připravuje úterý."],
  },
  ut: {
    rano: ["Úterý ráno — motor je zahřátý, teď se jede.", "Druhý den týdne a stále v sedle. Tak to má být.", "Úterý: pondělní chaos je za tebou, středa je daleko. Ideální čas na práci.", "Ráno v úterý voní po příležitostech.", "Úterý ráno je nejproduktivnější část týdne. Věří tomu spousta lidí. Buď jedním z nich.", "Po pondělí přichází úterý, a to je vlastně úleva.", "Dnes je úterý. Zákazníci jsou otevřenější než v pondělí.", "Rozhýbej se — úterní ráno čeká na tebe.", "Druhý den, druhá šance, stejné odhodlání.", "Úterý ráno: zásoby energie doplněny, CRM otevřen."],
    dopoledne: ["Úterní dopoledne — to je zlatá hodinka pro obchodníky.", "Dopoledne v úterý: zákazníci jsou v práci, kancelář je otevřená.", "Úterý dopoledne patří těm, co mají plán.", "Dvě dopoledne v týdnu jsou lepší než jedno. Toto je to druhé.", "Zavolej, napiš, navštiv — dopoledne v úterý je na to ideální.", "Máš rozjetý rytmus? Nepřerušuj ho.", "Zákazník, který v pondělí řekl 'zavolejte jindy', čeká právě teď.", "Dopoledne utíká — využij každou minutu.", "Úterní dopoledne: méně schůzek, více telefonátů.", "Kdo maká v úterý dopoledne, sklízí ve čtvrtek."],
    odpoledne: ["Úterní odpoledne — druhá vlna energie přichází.", "Po obědě to vždy trochu zpomalí. Ale ne u tebe.", "Odpoledne v úterý: ideální na nabídky a follow-upy.", "Zákazníci po obědě přemýšlejí. Pomoz jim přemýšlet správně.", "Odpoledne v úterý je čas na výsledky.", "Dvě hodiny odpoledne v úterý = čas na klíčový telefonát.", "Přišel jsi do odpoledne s přehledem. To je správně.", "Úterní odpoledne — ještě daleko do konce týdne, ale blíže k cíli.", "Každý follow-up teď může být zakázka na konci týdne.", "Úterý odpoledne: nekončí to dopolednem."],
    vecer: ["Úterý večer: týden nabírá tempo.", "Druhý den hotový. Jdeš dobře.", "Večer v úterý — čas na plánování zbytku týdne.", "Dva dny za tebou, tři před tebou. Dobrá bilance.", "Úterní večer patří těm, kdo si ho zaslouží.", "Zkontroluj úkoly, připrav se na středu.", "Co jsi dnes rozjel, zítra dokončíš.", "Úterní večer: krátký odpočinek před středním finiším.", "Dobrý úterní večer začíná dobrým pracovním úterkem.", "Dva dny, co počítají. Zbytek taky."],
  },
  st: {
    rano: ["Středa! Polovina týdne. Teď se rozhoduje, jak to dopadne.", "Střední bod týdne — odtud je stejně daleko dopředu i dozadu. Jdi dopředu.", "Středa ráno: půlmaraton je za tebou, druhá půlka začíná.", "Přežil jsi pondělí i úterý. Středa je tvoje.", "Středeční ráno voní po šanci otočit týden k lepšímu.", "Středa: den, kdy se rozhoduje páteční nálada.", "Polovina týdne je tady. Jak se ti daří?", "Středa ráno = čas přehodnotit, co funguje a co ne.", "Dobré ráno uprostřed týdne. Jsi v tom nejlepším místě.", "Středa je jako přestávka v zápase — jen kratší a bez odpočinku."],
    dopoledne: ["Středeční dopoledne — srdce pracovního týdne.", "Středa dopoledne: zákazníci jsou stále ještě v pracovním módu.", "Přesně uprostřed týdne. Přesně čas na klíčový hovor.", "Středeční dopoledne patří aktivním obchodníkům.", "Dopoledne ve středu: kdo nezavolá teď, čeká do příštího týdne.", "Středa dopoledne = ideální čas pro obchodní schůzku.", "Uprostřed týdne, uprostřed akce. Pojď!", "Středeční dopoledne: zákazníci jsou zahřátí a rozhodnutí.", "Polovina týdne je tady — a ty máš stále plno energie.", "Dopoledne uprostřed týdne: využij každou minutu."],
    odpoledne: ["Středeční odpoledne — svah dolů je klidnější. Ale ty jedeš stále nahoru.", "Po středeční přestávce na oběd přichází druhá vlna.", "Odpoledne ve středu: ještě dva celé dny do konce týdne.", "Středa odpoledne: čas na výsledky, ne na výmluvy.", "Středeční odpoledne: zákazníci jsou přístupnější než ráno.", "Druhá polovina středečního dne — teď to rozhoď.", "Čtvrtek se blíží, pátek taky. Ale nejdřív dodělej středu.", "Odpoledne uprostřed týdne: klíčový moment.", "Středa odpoledne je pro ty, co ráno nespali.", "Zákazník, který dopoledne řekl ne, odpoledne říká možná."],
    vecer: ["Středa večer: přežil jsi přelom týdne. Gratuluji.", "Polovina týdne za tebou. A zvládl jsi ji dobře.", "Středeční večer: zbývají jen dva pracovní dny. To dáš.", "Čtvrtek a pátek jsou před tebou — to je ta lepší polovina.", "Večer ve středu: čas na rychlý přehled a plán na čtvrtek.", "Středa večer = ten moment, kdy začínáš vidět konec tunelu.", "Dobré středeční večery mají ti, co pracovali.", "Dva dny do víkendu. Ale nejdřív si dej pohodu.", "Středa večer: ani pondělí, ani pátek. Přesně uprostřed.", "Výborný střed týdne = skvělý konec týdne."],
  },
  ct: {
    rano: ["Čtvrtek ráno — jeden den do finále. Makej!", "Čtvrteční ráno je pro ty, co se nevzdají před cílem.", "Čtvrtek: předpáteční energie. Využij ji.", "Ráno ve čtvrtek: zákazníci ještě stihnou říct ano před víkendem.", "Čtvrtek ráno = poslední šance na klíčový hovor před páteční pohodou.", "Předposlední den. Pořád dost času na velký deal.", "Čtvrteční ráno voní po uzavřených zakázkách.", "Dnes je čtvrtek. Zákazníci se začínají rozhodovat před víkendem.", "Čtvrtek ráno: ještě je čas všechno změnit.", "Čtvrteční ranní energie je nejvyšší v týdnu. Věř tomu."],
    dopoledne: ["Čtvrteční dopoledne: zákazníci chtějí vše vyřešit před víkendem.", "Dopoledne ve čtvrtek — deadline efekt funguje naplno.", "Čtvrtek dopoledne: kdo nezavolá teď, zavolá příští týden.", "Předvíkendový tlak? To je tvůj přirozený spojenec.", "Čtvrteční dopoledne: zákazníci jsou nejrozhodnější.", "Dnes dopoledne zavři, co jsi celý týden otevíral.", "Čtvrtek dopoledne = nejlepší čas na follow-up z pondělka.", "Zákazníci chtějí mít vše vyřešeno před víkendem. Pomoz jim.", "Čtvrteční dopoledne: jedna schůzka navíc může změnit vše.", "Dopoledne ve čtvrtek: sprint před cílem."],
    odpoledne: ["Čtvrteční odpoledne — ještě není pátek, ale je to blízko.", "Odpoledne ve čtvrtek: zákazníci jsou v nákupní náladě.", "Čtvrteční odpoledne: co nezavřeš dnes, zavřeš v pondělí.", "Zbývá jedno odpoledne a jeden celý den. Využij je.", "Čtvrtek odpoledne — příprava na páteční sprint.", "Odpoledne ve čtvrtek: čas na ty důležité hovory.", "Zákazníci v čtvrtek odpoledne rádi přijímají dobré nabídky.", "Čtvrteční odpoledne je zlatá hodinka pro uzavírání dealů.", "Jeden den do víkendu — ať se to počítá.", "Čtvrtek odpoledne: kdo maká teď, relaxuje v pátek."],
    vecer: ["Čtvrteční večer: zítra je pátek. To zní dobře, ne?", "Čtyři dny za tebou. Jeden zlatý před tebou.", "Večer ve čtvrtek — připrav se na páteční dojezd.", "Čtvrteční večer: zkontroluj pipeline, připrav schůzky.", "Zítra je pátek. Ale nejdřív dokonči čtvrtek.", "Čtyři dny odvedené práce. Zítřek bude vínová třešnička.", "Čtvrtek večer: plán na pátek je základ úspěchu.", "Jeden den do víkendu. Stojí to za jeden dobrý večer.", "Čtvrteční večer patří těm, co se těší na výsledky.", "Dobré ráno zítra začíná dobrým večerem dnes."],
  },
  pa: {
    rano: ["Pátek! Finišuj silně — víkend je odměna pro ty, co makali.", "Páteční ráno: poslední šance udělat z týdne něco výjimečného.", "Pátek ráno voní po uzavřených dealech a zaslouženém odpočinku.", "Pátý den, pátá rychlost. Jedeme!", "Páteční ráno: zákazníci chtějí mít vše hotovo před víkendem.", "Pátek ráno — dnes se počítá každý telefonát.", "Poslední pracovní ráno v týdnu. Udělej ho velkým.", "Pátek ráno je pro ty, co nevzdali ani v úterý.", "Páteční ráno: víkend vidíš na obzoru. Ale nejdřív jeden velký den.", "Pátek ráno — co neuzavřeš dnes, uzavřeš v pondělí. Volba je tvoje."],
    dopoledne: ["Páteční dopoledne: zákazníci chtějí mít vše vyřízeno před víkendem.", "Dopoledne v pátek — deadline efekt na maximum.", "Pátek dopoledne: zákazníci rozhodují rychle. Využij to.", "Poslední dopoledne týdne. Ať se to počítá.", "Páteční dopoledne je zlaté — zákazníci řeší věci, než odejdou.", "Dopoledne v pátek: uzavři, co jsi celý týden budoval.", "Pátek dopoledne = nejrychlejší rozhodování zákazníků.", "Kdo zavolá v pátek dopoledne, bývá příjemně překvapen.", "Páteční dopoledne: sprint před cílem.", "Dnes dopoledne máš šanci uzavřít skvělý týden."],
    odpoledne: ["Páteční odpoledne — ještě jsi v práci, a to o tobě leccos říká.", "Odpoledne v pátek: zákazníci jsou v pohodové náladě. Snad.", "Páteční odpoledne: co zbývá, to uzavři.", "Odpoledne v pátek je pro ty, co to myslí vážně.", "Pátek odpoledne: víkend je blízko, ale práce ještě ne.", "Poslední hodiny týdne. Udělejme je smysluplnými.", "Páteční odpoledne: zákazníci jsou přístupní a uvolnění.", "Odpoledne v pátek = čas na finální tahy.", "Kdo maká v pátek odpoledne, má čisté svědomí o víkendu.", "Páteční odpoledne: ať skončíš s úsměvem."],
    vecer: ["Páteční večer: týden za tebou, víkend před tebou. Zasloužíš si ho.", "Pět dní odvedené práce. Dnes večer odpočiň.", "Pátek večer — čas zavřít CRM a otevřít lednici.", "Dobrý týden? Skvělý. Špatný týden? Příští bude lepší.", "Páteční večer patří tobě. CRM může počkat do pondělí.", "Týden hotový. Co zbývá, vyřeší pondělí.", "Pátek večer: nejlepší část pracovního týdne.", "Pět dní, spousta hovorů, pár dealů. To je správný rytmus.", "Páteční večer — nezapomeň si poznamenat, co se povedlo.", "Dobrý víkend začíná dobrým pátečním večerem."],
  },
};
...
const getGreeting = (firstName) => {
  ...
  const actualKey = day === 0 || day === 6 ? "po" : ["", "po", "ut", "st", "ct", "pa"][day];
...
const Dashboard = ({ data, profile, onNavigate }) => {
  ...
  const won = deals.filter(d => d.status === "Vyhráno").reduce((sum, d) => sum + (d.value || 0), 0);
    ...
    <div>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 16, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
        <div style={{ padding: "12px 16px 8px", borderBottom: `1px solid ${C.border}` }}>
          ...
          {tasksToday.length === 0 && overdue.length === 0 && <div style={{ fontSize: 13, color: C.textDim, padding: "6px 0 4px" }}>Žádné úkoly na dnes 🎉</div>}
          {tasksToday.map(t => {
            const co = companies.find(c => c.id === t.company_id);
        ...
        <div style={{ padding: "10px 16px 12px" }}>
          ...
          {tasksNextWd.length === 0 && <div style={{ fontSize: 13, color: C.textDim }}>Žádné naplánované úkoly</div>}
          {tasksNextWd.map(t => {
            const co = companies.find(c => c.id === t.company_id);
            const color = taskTypeColors[t.type] || C.border;
            return (
      ...
      <div className="dash-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <div style={s.card}>
          ...
          {["Identifikováno", "Nabídka odeslána", "Jednání"].map(stage => {
            ...
            const val = items.reduce((sum, d) => sum + (d.value || 0), 0);
            const maxVal = Math.max(...["Identifikováno", "Nabídka odeslána", "Jednání"].map(st => deals.filter(d => d.status === st).reduce((sum, d) => sum + (d.value || 0), 0)), 1);
            return (
...
const Companies = ({ data, ops, focusId, onClearFocus }) => {
  ...
  const filtered = data.companies
    .filter(c => {
      ...
      return ms && (companyFilter === "Vše" || c.status === companyFilter);
    })
    .sort((a, b) => {
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", overflowX: "auto" }}>
        ...
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
          {["Vše", ...STATUSES.company].map(st => (
            <button key={st} onClick={() => setCompanyFilter(st)} style={{ ...s.btn(companyFilter === st ? "primary" : "ghost"), padding: "7px 11px", fontSize: 11 }}>{st}</button>
          ))}
        </div>
      </div>
      {filtered.map(c => {
        const lastNote = (c.notes || []).slice(-1)[0];
        const dealValue = data.deals.filter(d => d.company_id === c.id && !["Vyhráno", "Prohráno"].includes(d.status)).reduce((sum, d) => sum + (d.value || 0), 0);
        return (
...
const Deals = ({ data, ops }) => {
  const [modal, setModal] = useState(null);
  const [dealFilter, setDealFilter] = useState("Vše"); // přejmenováno z filter
  const filteredDeals = data.deals.filter(d => dealFilter === "Vše" || d.status === dealFilter);
  return (
    <div>
      <SectionHeader title="Pipeline" count={filteredDeals.length} onAdd={() => setModal("new")} addLabel="Přidat deal" />
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", overflowX: "auto" }}>
        {STATUSES.deal.map(st => {
          const items = data.deals.filter(d => d.status === st);
          if (!items.length) return null;
          return (
            <div key={st} onClick={() => setDealFilter(dealFilter === st ? "Vše" : st)} style={{ background: dealFilter === st ? C.accentLight : C.white, border: `1px solid ${dealFilter === st ? C.accent : C.border}`, borderRadius: 10, padding: "10px 14px", cursor: "pointer", transition: "all 0.15s" }}>
              ...
              <div style={{ fontSize: 11, color: C.textDim }}>{fmtMoney(items.reduce((sum, d) => sum + (d.value || 0), 0))}</div>
            </div>
          );
        })}
      </div>
      {filteredDeals.sort((a, b) => (b.value || 0) - (a.value || 0)).map(d => {
      ...
      {filteredDeals.length === 0 && <div style={{ textAlign: "center", padding: "40px 0", color: C.textDim }}>Žádné příležitosti</div>}
...
const Tasks = ({ data, ops, profile, onNavigateToCompany }) => {
  const [modal, setModal] = useState(null);
  const [gcalTask, setGcalTask] = useState(null);
  const [taskFilter, setTaskFilter] = useState("Aktivní"); // přejmenováno z filter
  const overdueCount = data.tasks.filter(t => isOverdue(t.date, t.status)).length;

  const sortDesc = taskFilter === "Dokončeno";
  const filteredTasks = data.tasks.filter(t => {
    if (taskFilter === "Aktivní") return ["Plánováno", "Probíhá"].includes(t.status);
    if (taskFilter === "Dnes") return t.date === today() && ["Plánováno", "Probíhá"].includes(t.status);
    if (taskFilter === "Po termínu") return isOverdue(t.date, t.status);
    if (taskFilter === "Dokončeno") return t.status === "Dokončeno";
    return true;
  }).sort((a, b) => sortDesc
  ...
  return (
    <div>
      <SectionHeader title="Moje úkoly" count={filteredTasks.length} onAdd={() => setModal("new")} addLabel="Přidat úkol" />
      <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
        {["Aktivní", "Dnes", "Po termínu", "Dokončeno", "Vše"].map(f => (
          <button key={f} onClick={() => setTaskFilter(f)} style={{ ...s.btn(taskFilter === f ? "primary" : "ghost"), fontSize: 12 }}>
        ...
        return (
          <div key={t.id} style={{ ...s.card, borderLeft: `3px solid ${od ? C.danger : STATUS_COLORS[t.status] || C.border}` }}>
            ...
            <div style={{ fontSize: 12, color: C.textMuted, display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 6 }}>
              {company && (
                <span onClick={e => { e.stopPropagation(); onNavigateToCompany(company.id); }}
      ...
      {filteredTasks.length === 0 && <div style={{ textAlign: "center", padding: "40px 0", color: C.textDim }}>Žádné úkoly</div>}
      {modal && <TaskModal
        initial={modal === "new" ? { title: "", type: "Telefonát", company_id: "", contact_id: "", date: today(), time: "", status: "Plánováno", note: "", owner_id: profile?.id } : data.tasks.find(t => t.id === modal)}
        companies={data.companies} contacts={data.contacts} profiles={data.profiles}
        onSave={async (f) => {
...
export default function App() {
  ...
  // ↓ Obnovení poslední stránky z localStorage
  const [page, setPage] = useState(() => {
    const saved = localStorage.getItem("vzv_last_page");
  ...
  useEffect(() => {
    const stored = getStoredSession();
    if (!stored?.access_token) { setAuthLoading(false); return; }
    if (isTokenExpired(stored)) {
      if (stored.refresh_token) {
        refreshSession(stored.refresh_token)
          .then(newSession => { storeSession(newSession); setSession(newSession); })
          .catch(() => { storeSession(null); })
          .finally(() => setAuthLoading(false));
      } else { storeSession(null); setAuthLoading(false); }
    } else { setSession(stored); setAuthLoading(false); }
  }, []);
    ...
    const loadProfile = async () => {
      try {
        ...
        if (profiles.length > 0) {
          ...
          const newProfile = { id: session.user.id, email: session.user.email, full_name: session.user.user_metadata?.full_name || session.user.email.split("@")[0], role: session.user.user_metadata?.role || "obchodnik" };
  ...
  // ↓ Navigace — ukládá poslední stránku do localStorage
  const navigate = (p, id = null) => {
    setPage(p);
    setFocusId(id);
    localStorage.setItem("vzv_last_page", p);
  };
